...
# Каталог для сохранения отчётов и графиков
REPORT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../report"))
os.makedirs(REPORT_DIR, exist_ok=True)

def plot_operation_times(results: Dict):
    """
    Строит графики времени выполнения операций (вставка, поиск, удаление)
    для каждой хеш-функции и типа хеш-таблицы в зависимости от коэффициента заполнения.
    """
...
def plot_collision_histogram(hash_table_class, hash_fn, num_keys=1000):
    """
    Строит гистограмму, показывающую распределение коллизий:
    для цепочек — длины бакетов, для открытой адресации — число проб при вставке.
    """
    ...
    if hash_table_class == HashTableChaining:
        ...
        for i, k in enumerate(keys):
            count = 0
            # Подсчёт количества проб до свободного слота
...
def generate_all_plots(results):
    # Построение графиков времени операций
    plot_operation_times(results)
    
    # Генерация гистограмм коллизий для разных хеш-функций и типов таблиц
