...
def measure_heap_operations():
    print("=== ИЗМЕРЕНИЕ ПРОИЗВОДИТЕЛЬНОСТИ ОПЕРАЦИЙ С КУЧЕЙ ===\n")
    print()
    ...
    print("Размер | Build (мс) | Послед. вставка (мс) | Извлечение всех (мс)")
    print("-" * 75)

    for size in sizes:
        array = random.sample(range(size * 10), size)

        # Построение кучи через heapify
    ...
    plt.subplot(1, 2, 1)
    plt.plot(sizes, build_heap_times, 'o-', label='Heapify (O(n))', linewidth=2)
    plt.plot(sizes, sequential_insert_times, 's-', label='Последовательная вставка (O(n log n))', linewidth=2)
    plt.xlabel('Количество элементов')
    plt.ylabel('Время (мс)')
    plt.title('Сравнение способов построения кучи')
    plt.legend()
    plt.grid(True, alpha=0.3)

    plt.subplot(1, 2, 2)
    plt.plot(sizes, extract_all_times, 'o-', label='Извлечение всех элементов', linewidth=2)
    plt.xlabel('Количество элементов')
    plt.ylabel('Время (мс)')
    plt.title('Производительность извлечения')
    plt.legend()
    plt.grid(True, alpha=0.3)
...
def compare_sorting_algorithms():
    ...
    print("Размер | Heapsort (мс) | Quicksort (мс) | Mergesort (мс) | Встроенная (мс)")
    print("-" * 85)
    ...
    for size in sizes:
        ...
        # Quicksort (простая рекурсивная реализация)
        def quicksort(arr):
            if len(arr) <= 1:
                return arr
            pivot = arr[len(arr) // 2]
        ...
        # Mergesort (рекурсивная реализация)
        def mergesort(arr):
            if len(arr) <= 1:
                return arr
            mid = len(arr) // 2
    ...
    plt.plot(sizes, builtin_sort_times, 'd-', label='Встроенная (Timsort)', linewidth=2)
