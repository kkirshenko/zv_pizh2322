...
def analyze_interval():
    """Исследование эффективности жадного выбора непересекающихся интервалов."""
    print("=== ИССЛЕДОВАНИЕ: ВЫБОР НЕПЕРЕСЕКАЮЩИХСЯ ИНТЕРВАЛОВ ===\n")
    ...
    print("Все доступные интервалы:")
    for it in intervals:
        print(f"  {it.name}: [{it.start}, {it.end}]")

    chosen = GreedyMethods.schedule_intervals(intervals)

    print("\nВыбранные жадным алгоритмом:")
    for it in chosen:
        print(f"  {it.name}: [{it.start}, {it.end}]")

    print(f"\nКоличество выбранных: {len(chosen)}")
...
def analyze_knapsack():
    """Сравнение жадного и точного подходов к задаче о рюкзаке."""
    print("\n=== ИССЛЕДОВАНИЕ: ЗАДАЧА О РЮКЗАКЕ ===\n")
    ...
    print("\n--- Случай, где жадный выбор не оптимален ---")
    capacity_small = 10
    tricky = [
        PackItem(60, 6, "X"),
        PackItem(50, 5, "Y"),
    ...
    for size in sizes:
        ...
        print(f"  Размер {size}: дробный={frac_times[-1]:.3f} мс, 0-1={zero_one_times[-1] if zero_one_times[-1] else 'N/A'} мс")

    return sizes, frac_times, zero_one_times

def analyze_huffman():
    """Оценка производительности и эффективности кодирования Хаффмана."""
    print("\n=== ИССЛЕДОВАНИЕ: КОД ХАФФМАНА ===\n")

    test_text = "this is an example for huffman encoding"
    print(f"Исходная строка: '{test_text}'")
    print(f"Длина: {len(test_text)} символов")
    print(f"Объём в ASCII: {len(test_text) * 8} бит")

    codes, encoded_text, tree = GreedyMethods.huffman_encode(test_text)
    print(f"\nЗакодированная строка: {encoded_text}")
    print(f"Объём после сжатия: {len(encoded_text)} бит")
    print(f"Степень сжатия: {len(encoded_text) / (len(test_text) * 8):.2%}")
    ...
    def print_tree(node, prefix="", is_left=True):
        ...
        char_repr = f"'{node.char}'" if node.char else "внутр."
...
def analyze_prim():
    """Проверка корректности и демонстрация работы алгоритма Прима."""
    print("\n=== ИССЛЕДОВАНИЕ: АЛГОРИТМ ПРИМА ===\n")
    ...
    total = 0
    for e in mst:
        print(f"  {e.u} — {e.v} (вес: {e.weight})")
        total += e.weight

    print(f"Суммарный вес дерева: {total}")

    return total, mst

def run_comprehensive_analysis():
    """Полный сравнительный анализ всех реализованных жадных алгоритмов."""
    print("=== КОМПЛЕКСНЫЙ СРАВНИТЕЛЬНЫЙ АНАЛИЗ ЖАДНЫХ АЛГОРИТМОВ ===\n")
    print()
    ...
    plt.plot(kn_sizes, frac_times, 's-', label='Дробный рюкзак', linewidth=2)
    ...
    plt.title('Эффективность кода Хаффмана')
    plt.grid(True, alpha=0.3)

    plt.subplot(2, 2, 3)
    x_pos = range(len(kn_sizes))
    plt.bar([x - 0.2 for x in x_pos], frac_times, 0.4, label='Дробный рюкзак', alpha=0.8)
    zero_one_available = [t if t else 0 for t in zero_one_times]
    plt.bar([x + 0.2 for x in x_pos], zero_one_available, 0.4, label='0-1 рюкзак (перебор)', alpha=0.8)
    plt.xlabel('Размер задачи')
    plt.ylabel('Время (мс)')
    plt.title('Рюкзак: точное vs жадное')
    ...
    plt.xlabel('Общее число интервалов')
    plt.ylabel('Выбрано жадным алгоритмом')
    plt.title('Масштабируемость интервального выбора')
    plt.grid(True, alpha=0.3)
    ...
    print("\nАнализ завершён. Графики сохранены в файл 'analysis.png'")
