...
### Выполненные задачи
- [x] Реализовать функцию линейного поиска элемента в массиве.
- [x] Реализовать функцию бинарного поиска элемента в отсортированном массиве.
- [x] Провести теоретический анализ сложности обоих алгоритмов.
- [x] Экспериментально сравнить время выполнения алгоритмов на массивах разного размера.
- [x] Визуализировать результаты, подтвердив асимптотику O(n) и O(log n).
...
(search_complexity_analysis.png)

## Выводы

В результате лабораторной работы реализованы и протестированы линейный и бинарный поиск. Эксперименты подтвердили теоретическую сложность: O(n) и O(log n) соответственно. Тестирование проводилось на массивах от 1 000 до 1 000 000 элементов с многократным усреднением и высокоточным замером времени. Бинарный поиск оказался в 24 453 раза быстрее линейного на массиве из 1 млн элементов. Графики, построенные с помощью matplotlib, наглядно подтверждают теоретические оценки. Работа показывает, что выбор алгоритма должен учитывать размер и упорядоченность данных.

## Контрольные вопросы

1. Асимптотическая сложность — оценка роста времени или памяти алгоритма при увеличении размера входа. Она позволяет сравнивать алгоритмы независимо от реализации и оборудования и прогнозировать поведение на больших данных.

2. Разница между O(1), O(n) и O(log n):
   
  – O(1): время не зависит от размера (доступ по индексу в массиве).
  – O(n): время растёт линейно (линейный поиск).
  – O(log n): время растёт логарифмически (бинарный поиск в отсортированном массиве).

3. Линейный перебирает все элементы; бинарный делит диапазон пополам на каждом шаге. Бинарный поиск требует, чтобы массив был отсортирован и допускал произвольный доступ к элементам.

4. Потому что O-нотация игнорирует константы, младшие члены и аппаратные факторы: кэширование, оптимизации компилятора, накладные расходы ОС и т.п. Эти факторы влияют на реальное время, но не на асимптотику.

5. Измерить время выполнения на данных разного размера, усреднить несколько прогонов, построить график зависимости времени от размера. При O(n) график линеен, при O(log n) — растёт очень медленно. Для чёткого подтверждения можно использовать логарифмические шкалы: O(n) даёт прямую с наклоном 1, O(log n) — прямую с наклоном 0.
