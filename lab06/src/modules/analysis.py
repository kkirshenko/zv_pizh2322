...
def build_balanced_tree(size):
    """
    Создание приближённо сбалансированного BST с использованием случайных значений.

    Общая сложность вставки: O(n log n) в среднем случае.
    """
    tree = BinTree()
...
def build_degenerate_tree(size):
    """
    Построение вырожденного BST путём вставки отсортированной последовательности.

    Сложность построения: O(n²) из-за рекурсивной вставки в худший случай.
    """
    tree = BinTree()
...
def build_degenerate_tree_iterative(size):
    """
    Эффективное создание вырожденного дерева без рекурсии (цепочка вправо).
    Сложность: O(n)
    """
    tree = BinTree()
...
def measure_find_time(tree, search_values, num_searches=1000):
    """
    Измерение среднего времени одного поиска (в микросекундах).
    Общая сложность: O(num_searches · h), где h — высота дерева.
    """
...
def run_analysis():
    ...
    print(f"Лимит глубины рекурсии: {sys.getrecursionlimit()}")
    print()
    ...
    for size in sizes:
        try:
            ...
            print(f"Пропуск размера {size} из-за превышения глубины рекурсии: {e}")
            continue
        except Exception as e:
            print(f"Ошибка при обработке размера {size}: {e}")
            continue
    ...
    if balanced_times and degenerate_times:
        ...
        plt.title('Влияние структуры дерева на время поиска')
        plt.legend()
        ...
        plt.plot(sizes, [size - 1 for size in sizes], '--', label='Линейная высота (n-1)', alpha=0.7)
        plt.plot(sizes, [1.44 * (size + 1).bit_length() for size in sizes], '--',
                 label='Теоретическая высота AVL (~1.44·log₂n)', alpha=0.7)
        plt.xlabel('Количество элементов')
        plt.ylabel('Высота дерева')
        plt.title('Зависимость высоты от размера дерева')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        print("\nАнализ завершён. Графики сохранены в файл 'bst_performance_analysis.png'")
    else:
